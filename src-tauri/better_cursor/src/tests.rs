...
use crate::{StructRead, StructWrite, BetterRead, BetterWrite};
use std::io::Result;
...
pub fn read_basic_values() {
    let buffer: Vec<u8> = vec![
        ...
        0x00, 0x00, 0x28, 0x41,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x34, 0x40
    ];
...
pub fn read_array_values() {
    ...
    assert_eq!(values.list, [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A]);
}
...
pub fn write_array_values() {
    ...
    assert_eq!(values.list, [0x0A, 0x09, 0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01]);
}
