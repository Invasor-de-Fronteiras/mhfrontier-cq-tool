...
pub fn load_strings() {
    ...
    assert_eq!(quest.strings.title, String::from("≪Ｇ★７花畑探索≫
百花の地に舞降りし華鳳鳥"));

    assert_eq!(quest.strings.clear_reqs, String::from("メインターゲットの達成"));
    assert_eq!(quest.strings.contractor, String::from("ハンターズギルド"));
    assert_eq!(quest.strings.description, String::from("花畑に≪フォロクルル≫がa
現れました。報告によれば蜜
を摂取し体質変化させる、
非常に珍しいモンスターのよ
うです。調査の協力をお願い
...
いる様子はなく危険は少ない
という報告も届いています。A"));
    assert_eq!(quest.strings.fail_reqs, String::from("３回力尽きる
タイムアップ"));
    assert_eq!(quest.strings.main_objective, String::from("フォロクルル１頭の狩猟"));
