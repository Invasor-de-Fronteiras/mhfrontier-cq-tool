...
export const QuestlistRow = ({ quest, showQuestlist }: QuestlistRowProps) =>
    <tr
        // key={`${quest.filename}-${quest.questId}`}
        className={classNames("hover:bg-emerald-300 cursor-pointer")}
    // onClick={() => setSelectedIndex(i === selectedIndex ? null : i)}
    >
        {showQuestlist && <td className="px-6 py-4" scope="row">{quest.filename}</td>}
        ...
        >
            ...
            <tbody>
                {questlist.map((quest, i) => {
                    return <QuestlistRow
                        key={`${i}${questlistIndex}-${quest.filename}-${quest.questId}`}
                        quest={quest}
...
export function QuestlistTab() {
    ...
    return (
        <div className="flex flex-row flex-wrap gap-2">
            <GroupCard title="Questlist">
                <Select
                    options={questlists.map((v, i) => ({ label: v.filename, value: i }))}
                    label="Questlist"
                    isClearable
                    onChange={item => {
                        if (item === null) return setSelected(null);
            ...
            <GroupCard title="Quests">
                <QuestlistQuests questlistIndex={selected} />
            </GroupCard>
        </div>
    );
}
